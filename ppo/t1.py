...
class LowerT1JoyStick:
    ...
    def reset(self, env):
        ...
        for i in range(self.diff, mj_model.nu):
            ...
            for name in self.cfg["init_state"]["default_joint_angles"].keys():
                if name in mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i):
                    self.default_dof_pos[i - self.diff] = self.cfg["init_state"][
            ...
            for name in self.cfg["control"]["stiffness"].keys():
                if name in mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i):
                    self.dof_stiffness[i - self.diff] = self.cfg["control"][
                        "stiffness"
                    ][name]
                    self.dof_damping[i - self.diff] = self.cfg["control"]["damping"][
                        name
                    ]
                    found = True
            if not found:
        ...
        mj_data.qpos[: self.qpos_shape] = np.concatenate(
            [
                ...
                np.array(
                    self.cfg["init_state"]["rot"][3:4]
                    + self.cfg["init_state"]["rot"][0:3],
                    dtype=np.float32,
                ),
    ...
    def get_obs(self, command, obs, info):
        ...
        if info is None or "robot_quat" not in info:
            quat = np.array([0.0, 0.0, 0.0, 1.0])  # default identity quaternion
        ...
        obs[11:23] = (dof_pos - self.default_dof_pos) * self.cfg["normalization"][
            "dof_pos"
        ]
    ...
    def get_actions(self, command, observation, info):
        ...
        observation = np.array(observation)  # Ensure it's numpy array
        ...
        ctrl = np.clip(
            self.dof_stiffness * (self.dof_targets - dof_pos)
            - self.dof_damping * dof_vel,
            mj_model.actuator_ctrlrange[self.diff :, 0],
            mj_model.actuator_ctrlrange[self.diff :, 1],
        )

        self.it += 1
        self.gait_process = np.fmod(
